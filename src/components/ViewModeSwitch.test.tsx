import { fireEvent, render, screen } from '@testing-library/react'
import { describe, expect, it, vi } from 'vitest'
import { ViewModeSwitch } from './ViewModeSwitch'

describe('ViewModeSwitch', () => {
  it('exposes the Cosmic Filament exploration modes and changes the active mode', () => {
    const onChange = vi.fn()
    render(<ViewModeSwitch value="all" onChange={onChange} />)
    expect(screen.getByRole('button', { name: '모두' })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByRole('button', { name: '연결' })).toBeEnabled()
    expect(screen.getByRole('button', { name: '접점만' })).toBeEnabled()
    fireEvent.click(screen.getByRole('button', { name: '접점만' }))
    expect(onChange).toHaveBeenCalledWith('overlap')
  })
})
