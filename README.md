...
> 생각은 하나의 빛이고, 색인은 빛이 만나는 지점이다. 그 사이의 연결이 필라멘트를 이룬다.
...
![공개용 예시 데이터에서 생각이 만나는 지점을 밝힌 Cosmic Filament 화면](docs/images/cosmic-filament-meetings.png)

_공개용 가상 데이터에서 여러 생각이 만나는 지점을 밝힌 화면입니다._

## 세계관

- **생각(Thought)** — 어둠 속에 놓인 하나의 빛
- **색인(Index)** — 여러 생각이 만나는 지점
...
- **겹침**: 둘 이상의 생각이 공유하는 색인과 그 생각만 보기
- **만남**: 현재 화면은 그대로 두고, 여러 생각이 만나는 색인과 연결을 밝게 표시
